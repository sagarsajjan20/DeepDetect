...
const Hero = ({ openAuthModal }) => {
  ...
  const analyzeImage = async () => {
    ...
    try {
      ...
      // Make API call
      const response = await fetch("https://deepdetect-s9ar.onrender.com/api/predict", {
        method: "POST",
        body: formData,
        headers: {
      ...
      setAnalysisResult({
        ...
        details: result.prediction === "Fake" 
              ...
              <div
                className={`border-4 border-dashed rounded-2xl p-12 text-center transition-all ${
                  isDragging
                    ...
                    : "border-gray-400 dark:border-gray-700 hover:border-blue-400 dark:hover:border-blue-500"
                }`}
              ...
              >
                <div className="flex flex-col items-center">
                  ...
                  <label
                    ...
                    className="px-8 py-3 bg-blue-600 text-white text-lg font-medium rounded-lg hover:bg-blue-700 transition-all cursor-pointer"
                  >
                    Upload Image
                  </label>
                </div>
                  ...
                  {analysisResult && (
                    <div
                      className={`absolute top-6 right-6 px-4 py-2 rounded-full text-white text-lg font-medium ${
                        analysisResult.error? analysisResult.isValid? "bg-yellow-500":"bg-blue-500": analysisResult.isFake ? "bg-red-500" : "bg-green-500"
                      }`}
                    >
                      {
                      analysisResult.error? analysisResult.isValid? "Uanble To Predict":"Unauthorized": analysisResult.isFake ? "Likely Fake" : "Likely Real"
                      }
                    </div>
                  )}
                </div>
  ...
  <button
    ...
    className="w-full px-6 py-4 bg-blue-600 text-white text-xl font-semibold rounded-lg hover:bg-blue-700 transition-all flex items-center justify-center gap-2"
